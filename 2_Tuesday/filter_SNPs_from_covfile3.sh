...
# NOTE: this version should work with a g.vcf file outputted from GATK haplotypecaller
...
# here we remove the monomorphic blocks if we are inputting g.vcf (which have "<NON_REF>" in the 5th column)
zcat $2 | awk '$5!="<NON_REF>"' > tmp0
...
bedtools intersect -a tmp.bed -b tmp0 -v > tmp2.bed
